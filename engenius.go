...
func ConfigureTeamWifiEngenius(red1, red2, red3, blue1, blue2, blue3 *Team) error {
	...
	replaceSsid := func(team *Team, vlan int, iface int) {
		...
		addSsidsCommand += fmt.Sprintf("wless2 network ssidp %d vid %d\n", iface, vlan)						// Set VLAN
		addSsidsCommand += fmt.Sprintf("wless2 network ssidp %d apsecu 7\n", iface)							// Enable PSK + TKIP/AES security
		addSsidsCommand += fmt.Sprintf("wless2 network ssidp %d apsecu 7 passp %s\n", iface, team.WpaKey)	// Set WPA key
		addSsidsCommand += fmt.Sprintf("wless2 network ssidp %d apsecu 7 accept\n", iface)					// Commit security changes
	}
